...
class SellerAuctionService {
  ...
  async getAuctionById(id, sellerId) {
    try {
      const auction = await prisma.auction.findUnique({
        where: { id, sellerId }, // Ensure the auction belongs to the seller
        include: {
  ...
  async getSellerAnalytics(startDate, endDate) {
    try {
      const where = {
        createdAt: {
  ...
  async getSellerDashboardStats(sellerId) {
    try {
      const where = { sellerId }; // Filter by seller ID
      ...
      const [
        ...
        prisma.auction.count({
          where: { ...where, status: 'ACTIVE', endTime: { gt: new Date() } },
        }),
        prisma.auction.count({
          where: { ...where, status: 'SOLD' },
        }),
      ...
      logger.error('Error in getSellerDashboardStats:', error);
      throw new AppError(500, 'Failed to fetch dashboard stats');
    }
  }
}

module.exports = new SellerAuctionService();
